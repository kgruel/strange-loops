"""Run the benchmark with exec-generated flat verify, applied in-memory.

Reads the HEAD version of benchmark_replay_fidelity.py, replaces
_prebuild_all/run_verification/main with the optimized versions,
and execs the result — all within a single process, immune to
concurrent file modifications.

Usage: uv run python benchmarks/_run_patched.py
"""
import re
import sys
import os

# Restore HEAD version first (in case it was modified)
os.system("git -C /Users/kaygee/Code/loops-autoresearch checkout -- benchmarks/benchmark_replay_fidelity.py 2>/dev/null")

TARGET = os.path.join(os.path.dirname(__file__), "benchmark_replay_fidelity.py")
...
if not m:
    print("ERROR: Could not find _prebuild_all")
    sys.exit(1)

# Also check for _make_apply before _prebuild_all
...
patched_src = src[:cut_pos] + NEW_TAIL.lstrip('\n')

# Verify syntax
try:
    code = compile(patched_src, TARGET, 'exec')
except SyntaxError as e:
    print(f"Syntax error in patched code at line {e.lineno}: {e.msg}")
    sys.exit(1)

# Write the patched version to disk for committing later
with open(TARGET, 'w') as f:
    f.write(patched_src)

# Execute the patched version
