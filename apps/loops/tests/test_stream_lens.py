...
def test_stream_tick_error():
    assert "error msg" in _text(
        stream_view({"_tick_error": "error msg"}, Zoom.SUMMARY, 80)
    )


def test_stream_minimal():
    t = _text(
        stream_view({"facts": [_fact("metric"), _fact("status")]}, Zoom.MINIMAL, 80)
    )
...
def test_stream_summary():
    t = _text(
        stream_view(
            {"facts": [_fact("decision", {"topic": "auth", "message": "Use JWT"})]},
            Zoom.SUMMARY,
            80,
        )
    )
...
def test_stream_summary_with_key_field():
    data = {
        "facts": [_fact("metric", {"name": "cpu"})],
        "fold_meta": {"metric": {"key_field": "name"}},
    }
...
def test_stream_detailed_with_id():
    t = _text(
        stream_view(
            {"facts": [_fact("m", {"name": "x"}, id="01ABCDEFGHIJKLMNO123456789")]},
            Zoom.DETAILED,
            80,
        )
    )
...
def test_stream_detailed_shows_long_fields():
    # record_line's DETAILED density: only long/notable fields as continuation
    # (short secondary fields like "extra" surface at FULL — see below). A long
    # message renders on a continuation line.
    long_msg = "Chose SQLite over filesystem for atomic writes and query support"
    t = _text(
        stream_view(
            {"facts": [_fact("decision", {"topic": "store", "message": long_msg})]},
            Zoom.DETAILED,
            80,
        )
    )
...
def test_stream_full_shows_secondary_fields():
    # Short secondary fields surface at FULL (record_line FULL renders every field).
    t = _text(
        stream_view(
            {"facts": [_fact("m", {"name": "x", "status": "open", "extra": "val"})]},
            Zoom.FULL,
            80,
        )
    )
    assert "extra:" in t and "status:" in t
        ...
        stream_view(
            {
                "facts": [
                    _fact(
                        "m", {"name": "x"}, id="01ABC", observer="alice", origin="proj"
                    )
                ]
            },
            Zoom.FULL,
...
def test_stream_id_lookup():
    t = _text(
        stream_view(
            {
                "facts": [_fact("m", {"name": "x"}, id="01ABC", observer="a")],
                "_id_lookup": True,
            },
            Zoom.SUMMARY,
            80,
...
def test_stream_no_width():
    assert (
        stream_view({"facts": [_fact("m", {"name": "x"})]}, Zoom.SUMMARY, None)
        is not None
    )
...
def test_stream_tick_no_facts():
    assert (
        stream_view({"facts": [], "_tick": {"index": 0, "total": 5}}, Zoom.SUMMARY, 80)
        is not None
    )


def test_stream_summary_key_not_in_labels():
    assert "X" in _stream_summary(
        {"custom_id": "X", "name": "foo"}, key_field="custom_id"
    )
