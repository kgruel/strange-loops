"""Demo script exercising all Block composition operations.
...
def main() -> None:
    ...
    title = Block.text("Hello, Blocks!", title_style)
    ...
    wrapped = Block.text("hello world", body_style, width=8, wrap=Wrap.WORD)
    ...
    char_wrap = Block.text("abcdefghij", body_style, width=4, wrap=Wrap.CHAR)
    ...
    ellipsis_block = Block.text("truncated text here", body_style, width=10, wrap=Wrap.ELLIPSIS)
    ...
    none_trunc = Block.text("hello world", body_style, width=5, wrap=Wrap.NONE)
    ...
    # --- 6. Horizontal join ---
    left = Block.text("LEFT", Style(fg="green", bold=True))
    right = Block.text("RIGHT", Style(fg="red", bold=True))
    ...
    # --- 7. Vertical join ---
    top_block = Block.text("top line", Style(fg="blue"))
    bot_block = Block.text("bottom line", Style(fg="magenta"))
    ...
    # --- 8. Truncate ---
    long_block = Block.text("a very long piece of text", body_style)
    ...
    # No-op truncate
    short_block = Block.text("hi", body_style)
    ...
    header = Block.text("Status Panel", Style(fg="cyan", bold=True))
    separator = Block.text("─" * 14, Style(fg="yellow", dim=True))
    body = Block.text("All systems OK", Style(fg="green"))
    ...
    tall_block = Block.text("line 1", body_style, width=8, wrap=Wrap.NONE)
    tall_block_2 = join_vertical(
        Block.text("A", Style(fg="red")),
        Block.text("B", Style(fg="green")),
        Block.text("C", Style(fg="blue")),
    )
    ...
    for name, chars in [("ROUNDED", ROUNDED), ("HEAVY", HEAVY),
        ...
        b = border(Block.text(f" {name} ", body_style), chars, Style(fg="cyan"))
    ...
    label = Block.text("Render", Style(fg="green", bold=True))
    value = Block.text("OK", Style(fg="cyan"))
    status_line = join_horizontal(label, Block.text(": ", dim_style), value)
