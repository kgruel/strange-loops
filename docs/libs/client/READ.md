...
| **`search_facts`** | Full-text search (FTS) queries over payload contents. | `SearchResult` |
...
### `read_summary(target)`
Returns high-level storage metrics, kind distribution, and index agreement:
...
| **`TimelineResult`** | `loops.cli/timeline-result/v1` | `events: list[dict]`, `start_ts`, `end_ts` |
| **`SyncResult`** | `loops.cli/sync-result/v1` | `status`, `indexed_facts`, `duration_ms` |
