...
def fold_view(data: "FoldState", zoom: Zoom, width: int | None, **kwargs) -> Block:
    ...
    # --- Protocol ---
    # Resolve vertex target for emit commands — use the vertex name from fold state
    vertex_target = getattr(data, "vertex", None) or kwargs.get("vertex_name", "autoresearch")

    lines.append("## Protocol")
    lines.append("")
    lines.append("You are one iteration of an autonomous optimization loop. Your job:")
    lines.append("try ONE idea, measure it, emit the result, and exit.")
    lines.append("The system launches the next iteration automatically.")
    lines.append("")
    ...
    lines.append(f"   loops emit {vertex_target} experiment \\")
    ...
    lines.append("   ```")
    lines.append("")
    lines.append("8. **Exit.** Do NOT loop, repeat, or start another iteration.")
