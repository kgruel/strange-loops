...
The one-line thesis, in the store's own words
(`docs/scratch/080-overnight/store-dumps/facts-60d.txt:9`, session fact
...
| **tasked** (successor/sibling) | `/Users/kaygee/Code/tasked/src/tasked/` | Separate repo; its `substrate.py` and `walker.py` cite ticked as lineage ("the ticked reset=true trap", substrate.py:90; "ticked's field lesson", walker.py:225; "subtask/ticked lineage", adapters.py:4). It has its own 2s-poll daemon (the "walker"). |
...
`docs/scratch/080-overnight/store-dumps/frictions.txt:91`
...
(`docs/scratch/080-overnight/store-dumps/frictions.txt:213`,
...
The prompt's stronger attribution — that *this docstring* says deletion waits
for both daemon access **and conditional emit** — is false. The docstring names
only the upstream store-path/client API. The two-part done bar appears instead
in `docs/scratch/080-overnight/store-dumps/thread-tasked-forcing.txt:3`:
“When 1+2 land, tasked's substrate.py deletes as its docstring predicts,” where
item 1 is daemon-shaped access and item 2 is conditional emit/CAS. That store
fact interprets the deletion bar; it is not text present in the docstring.
...
`docs/scratch/080-overnight/store-dumps/facts-60d.txt:41`
...
These are **not yet a complete resumable cursor API**. The three read methods
filter on rowid but do not return rowids with their facts; `append()` returns a
fact ULID, not its rowid (`sqlite_store.py:474-522`); and `total` is a row count,
not explicitly `MAX(rowid)`. Thus the generic `replay()` helper's returned
`store.total` is usable as a next rowid cursor only while the table's rowids
remain contiguous. The WAL-incremental fix-shape has most query machinery, but
still needs an explicit cursor-bearing batch/append contract as well as vertex
composition.
...
### 4.3 Change-feed / notification primitives — local pieces, no SQLite wake

Grep across `libs/` for `update_hook`, `data_version`, `inotify`,
`watchdog`, `kqueue`, `fsevents`: zero hits. There is no
cross-process subscription or wake primitive for SQLite in the engine.
However, saying there is *no change-detection primitive at all* would be
false. Three narrower pieces exist:

1. `EventStore.version` is “bumped on each append” for in-process consumers to
   detect new events (`libs/engine/src/engine/store.py:76-85,123-138`). It does
   not observe a second process or wake an event loop.
2. `Projection.advance(store)` pulls `store.since(self.cursor)` and folds only
   new events in O(new events) (`libs/engine/src/engine/projection.py:1-10,
   24-39,100-111`). It is a generic single-projection mechanism, not wired to
   `Vertex`, `VertexProgram`, SQLite change delivery, boundary suppression, or
   changed-row output; production grep finds no call sites.
3. The incremental-tail primitive for JSONL files, not SQLite, is
   `libs/engine/src/engine/tailer.py:1-12`:
...
describes. There is no cached `FoldState`, partial vertex re-fold, or
fold-since-cursor on `VertexProgram`/`vertex_read`/`vertex_fold`. The generic
`Projection.advance(store)` noted in §4.3 is the exception at a lower layer;
it has not been composed into the vertex read lifecycle.
...
| Change notification / wake | 2s sleep-poll | "on append: apply → diff → emit" | "rows lighting as facts land" | no SQLite/cross-process wake; in-memory `EventStore.version`, generic `Projection.advance`, and JSONL `Tailer.poll()` only |
