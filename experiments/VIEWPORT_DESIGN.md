...
Cells has mouse scroll events (`MouseEvent.scroll_delta`) and components like `ListState` and `TableState` that embed scroll management. The scroll logic is duplicated:
...
`libs/cells/src/cells/viewport.py` — new file in the CLI core layer (no TUI dependencies).

Export from `cells/__init__.py` alongside `Block`, `Span`, etc.
