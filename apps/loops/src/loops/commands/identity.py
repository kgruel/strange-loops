...
def validate_emit(vertex_path: Path, observer: str, kind: str) -> str | None:
    ...
    Checks:
    1. Observer is declared in the vertex itself or .vertex chain (project -> global)
    ...
    all_observers = list(vertex_observers) + list(project_observers) + list(global_observers)
